...
export default function StatCard({ label, value, variant = 'default', sublabel }: StatCardProps) {
  return (
    <div className={`bg-slate-900 rounded-lg border-l-4 ${borderColor[variant]} p-4 flex flex-col justify-between`}>
      <p className="text-slate-400 text-xs uppercase tracking-wider leading-tight">{label}</p>
      <p className={`text-3xl font-bold mt-2 ${valueColor[variant]}`}>
